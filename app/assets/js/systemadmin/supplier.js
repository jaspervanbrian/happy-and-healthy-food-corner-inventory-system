$(document).ready(function() {
    ...
    function initializeSupplierList() {
        var suppliers = $.ajax({
        ...
        suppliers.done(function(response, textStatus, jqXHR) {
            ...
            if (supplier_list.length > 0) {
                $.each(supplier_list, function(i, supplier) {
                    $("#supplier_list").append('<option value="' + supplier.id + '">' + supplier.name + '</option>')
                });
            }
        });
        ...
        requestSupplierList.done(function (response, textStatus, jqXHR){
            ...
            if (suppliers.length === 0) {
                ...
                $.each(suppliers, function(i, supplier) {
                	$supplierRows.append('<tr> <td>' + supplier.name + '</td><td>' + supplier.location + '</td><td>' + supplier.contact_number + '</td><td> <button data-toggle="modal" data-target="#edit_supplier' + supplier.id + '" class="btn btn-outline-primary"><span class="fa fa-edit"></span></button><button data-toggle="modal" data-target="#delete_supplier' + supplier.id + '" class="btn btn-outline-danger"><span class="fa fa-times"></span></button></td></tr>');
                    $("#edit_supplierModals").append('<div class="modal fade" id="edit_supplier' + supplier.id + '" tabindex="-1" role="dialog" aria-labelledby="edit_supplierModal' + supplier.id + '" aria-hidden="true"> <div class="modal-dialog" role="document"> <div class="modal-content"> <form action="../../controllers/admin/EditSupplier.php" method="post"> <div class="modal-header"> <h5 class="modal-title" id="edit_supplierModal' + supplier.id + '">Edit Supplier</h5> <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true" class="fa fa-times"></span> </button> </div><div class="modal-body"> <div class="container"> <div class="row"> <div class="col-3"></div><div class="col-6"> <small>Name <span class="text-danger">*</span></small> <input type="text" name="name" value="' + supplier.name + '" class="form-control" required> </div></div><div class="row"> <div class="col-3"></div><div class="col-6"> <small>Location <span class="text-danger">*</span></small> <input type="text" name="location" value="' + supplier.location + '" class="form-control" required> </div></div><div class="row"> <div class="col-3"></div><div class="col-6"> <small>Contact number <span class="text-danger">*</span></small> <input type="number" name="contact_number" value="' + supplier.contact_number + '" class="form-control" required> </div></div></div></div><div class="modal-footer"> <input type="hidden" name="supplier_id" value="' + supplier.id + '"> <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button> <button type="submit" class="btn btn-danger">Edit</button> </div></form> </div></div></div>');
	...
	function getSuppliersPages(data) {
        ...
        requestPages.done(function(response, textStatus, jqXHR) {
            if (parseInt(response) > 5) {
                ...
                totalPages = Math.ceil((parseInt(response)/5));
